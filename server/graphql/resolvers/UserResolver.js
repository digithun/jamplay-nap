...
const forget = async ({ context, args }) => await context.nap.willResetPassword(context, args.email).catch(onError(context))

const resetPassword = async ({ context, args }) => await context.nap.willChangePasswordByToken(args.email, args.token).catch(onError(context))
