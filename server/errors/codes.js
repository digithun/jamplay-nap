...
module.exports = {
  ...
  get AUTH_INVALID_ARGUMENT () {
    return new GenericError('auth/invalid-argument', 'An invalid argument was provided to an Authentication method. The error message should contain additional information.')
  },
  ...
  get AUTH_INVALID_PASSWORD () {
    return new GenericError('auth/invalid-password', 'The provided value for the password user property is invalid. It must be a string with at least six characters.')
  },
  ...
  get AUTH_OPERATION_NOT_ALLOWED () {
    return new GenericError('auth/operation-not-allowed', 'The provided sign-in provider is disabled for your NAP project. Enable it from the Sign-in Method section of the NAP console.')
  },
  ...
  get AUTH_PROJECT_NOT_FOUND () {
    return new GenericError('auth/project-not-found', 'No NAP project was found for the credential used to initialize the Admin SDKs. See Add NAP to your app for documentation on how to generate a credential for your project and use it to authenticate the Admin SDKs.')
  },
  ...
  get AUTH_APP_NOT_AUTHORIZED () {
    return new GenericError('auth/app-not-authorized', "The app identified by the domain where it's hosted, is not authorized to use NAP Authentication with the provided API key. Review your key configuration in the Google API console.")
  },
  ...
  get AUTH_REQUIRES_RECENT_LOGIN () {
    return new GenericError('auth/requires-recent-login', "The user's last sign-in time does not meet the security threshold. Use firebase.User#reauthenticateWithCredential to resolve. This does not apply if the user is anonymous.")
  },
  get AUTH_TOO_MANY_REQUESTS () {
    return new GenericError('auth/too-many-requests', 'Requests are blocked from a device due to unusual activity. Trying again after some delay would unblock.')
  },
  get AUTH_UNAUTHORIZED_DOMAIN () {
    return new GenericError('auth/unauthorized-domain', 'The app domain is not authorized for OAuth operations for your NAP project. Edit the list of authorized domains from the NAP console.')
  },
  get AUTH_USER_DISABLED () {
    return new GenericError('auth/user-disabled', 'The user account has been disabled by an administrator. Accounts can be enabled or disabled in the NAP Console, the Auth section and Users subsection.')
  },
  get AUTH_USER_TOKEN_EXPIRED () {
    return new GenericError('auth/user-token-expired', "The user's credential has expired. This could also be thrown if a user has been deleted. Prompting the user to sign in again should resolve this for either case.")
  },
  ...
  get AUTH_WRONG_PASSWORD () {
    return new GenericError('auth/wrong-password', 'Password is invalid for the given email, or if the account corresponding to the email does not have a password set.')
  },
  ...
  get AUTH_PROVIDER_ALREADY_LINKED () {
    return new GenericError('auth/provider-already-linked', "Thrown if the provider has already been linked to the user. This error is thrown even if this is not the same provider's account that is currently linked to the user.")
  }
}
