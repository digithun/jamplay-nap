...
module.exports = {
  ...
  AUTH_PROJECT_NOT_FOUND: new GenericError('auth/project-not-found', 'No NAP project was found for the credential used to initialize the Admin SDKs. See Add NAP to your app for documentation on how to generate a credential for your project and use it to authenticate the Admin SDKs.'),
  ...
  AUTH_APP_DELETED: new GenericError('auth/app-deleted', "Thrown if the instance of NAPApp has been deleted."),
  AUTH_APP_NOT_AUTHORIZED: new GenericError('auth/app-not-authorized', "Thrown if the app identified by the domain where it's hosted, is not authorized to use NAP Authentication with the provided API key. Review your key configuration in the Google API console."),
  AUTH_ARGUMENT_ERROR: new GenericError('auth/argument-error', "Thrown if a method is called with incorrect arguments."),
  AUTH_INVALID_API_KEY: new GenericError('auth/invalid-api-key', "Thrown if the provided API key is invalid. Please check that you have copied it correctly from the NAP Console."),
  ...
  AUTH_UNAUTHORIZED_DOMAIN: new GenericError('auth/unauthorized-domain', "Thrown if the app domain is not authorized for OAuth operations for your NAP project. Edit the list of authorized domains from the NAP console."),
  AUTH_USER_DISABLED: new GenericError('auth/user-disabled', "Thrown if the user account has been disabled by an administrator. Accounts can be enabled or disabled in the NAP Console, the Auth section and Users subsection."),
